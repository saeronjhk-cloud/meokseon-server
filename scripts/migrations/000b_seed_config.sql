-- ============================================================================
-- 000b_seed_config.sql — Config 씨드 (멱등 래퍼)  세션47 신설
-- ============================================================================
-- ★ 왜 새 파일인가
--   `002_seed_config.sql` 은 평범한 INSERT 다. 두 번 돌리면
--   `idx_config_unique` 에 걸려 duplicate key 로 죽는다.
--   지금까지는 psql 호출에 `-v ON_ERROR_STOP=1` 이 없어 그 실패가 **보이지 않았다**
--   (= 세션46 이 지적한 「거짓 초록」). ON_ERROR_STOP 을 켜는 순간 002 는 재실행이 불가능해진다.
--
-- ★ 무엇을 하나
--   002 의 본문을 **글자 하나 바꾸지 않고** 그대로 담되,
--   `nutrition_config` 이 비어 있을 때만 실행한다. 이미 씨드가 있으면 통째로 건너뛴다.
--   → 몇 번을 돌려도 안전하다. 002 는 역사로 그대로 남는다(수정하지 않았다).
--
-- ⚠ 이 파일은 002 의 **사본**이다. 002 는 동결된 역사라 바뀌지 않으므로 드리프트는 없다.
--   기준값을 바꿀 일이 생기면 002/000b 가 아니라 **새 마이그레이션**을 쓸 것.
--
-- 실행:  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f scripts/migrations/000b_seed_config.sql
-- ============================================================================

DO $seed$
BEGIN
  IF EXISTS (SELECT 1 FROM nutrition_config) THEN
    RAISE NOTICE '000b_seed_config: nutrition_config 에 이미 행이 있습니다 — 씨드를 건너뜁니다.';
    RETURN;
  END IF;
    -- ============================================================
