...
const REAL_DROPPED = [
  ...
  '310g(고형량155g)', '식품', '.', '젤란검)',
  // ★★ 세션57 — sentinel GT 에서 «이관»된 두 질문이다. 지운 것이 아니다.
  //   원래 `P-대두-레시틴`(원재료명: 레시틴, 정제수) · `P-쇠고기-젤라틴`(원재료명: 젤라틴, 설탕)이
  //   원재료 경로에서 「붙이지 않는다」를 지키고 있었는데, 그 경로가 폐기(D55-2)되면서 갈 곳이 없어졌다.
  //   질문 자체는 살아 있다 — 판별기 A(사용자 자유입력·HACCP `allergy` 필드)에는 이 이름이 실제로 들어온다.
  //   제이 결정 2026-08-07:
  //     · 수식어 없는 `레시틴` 은 대두로 단정하지 않는다 (해바라기·난황 레시틴이 있다).
  //       대두 유래면 라벨에 `대두` 가 따로 인쇄되고 그것은 이미 잡는다 → 과소경고가 아니다.
  //     · 축종 없는 `젤라틴` 은 어떤 축종도 붙이지 않는다 (돈피·우피·어류 부산물).
  //       무조건 쇠고기로 붙이면 돈피 젤라틴에서 «두 방향으로 동시에» 틀린다.
  //   ⚠ `레시틴(대두)` `대두레시틴` `돈피젤라틴` 은 위 REAL_PAIRS 가 그대로 붙인다 — 그쪽을 지우지 말 것.
  '레시틴', '젤라틴',
];
