...
const KNOWN = {
  ...
  //  `src/services/allergenName.js` 의 별칭·경계 가드 문제다. 세션50 은 그 파일을
  //  편집하지 않는다(다른 에이전트 소관). 여기서 조각을 어떻게 잘라도 결과가 같다.
  //  ★ 수정 전에는 15자 필터가 이 조각을 통째로 버려 **드러나지 않던** 오검출이다.
  //    길이 필터를 걷어낸 대가 = L4 증가분. 숨기지 않고 대장에 올려 둔다.
  'X-fp-mil': {
    owner: 'allergenName.js',
    defect: 'X2',
    note: "'밀폐'·'밀봉'→밀. `밀` 에는 경계 가드가 없다(allergenName.js 주석: `밀크` 를 막으면 "
      + '`통밀크래커` 의 밀까지 잃어 과소경고가 된다). 별칭표에 부정 가드를 추가할지는 그 파일 소관.',
  },
};
