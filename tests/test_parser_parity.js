...
/**
 * ★★ 세션45 — 성능 단정을 **한 번의 콜드 런**으로 재면 안 된다.
 *
 * 무엇이 문제였나 (세션45 실측) —
 *   §6 「치명2」 가 약 40% 확률로 실패하고 있었다. 원인은 ReDoS 재발이 아니라 **측정 방법**이다.
 *   최악 입력 `함유런`(`'밀 함유\n'.repeat(1414)` = 1,765자, 알레르기 세그먼트 1,414개)의
 *   10회 실측: 최솟값 5.9 / 중앙값 7.8 / **최댓값 42.6 ms**. 최댓값만 JIT 컴파일 때문에 튄다.
 *
 * 선형임을 어떻게 확인했나 (상한을 올리려면 근거를 댈 수 있어야 한다 — §9 규칙) —
 *   같은 입력을 1×·2×·4× 로 늘려 중앙값이 7.8 → 15.1 → 23.5 ms.
 *   길이에 **비례**한다. 역행추적이면 2배에서 4배가 아니라 수백~수천 배로 뛴다
 *   (실제 사례: 411바이트 18,055 ms). 즉 이 입력은 그냥 무거운 선형 작업이다.
 *
 * 그래서 **측정을 먼저 고친다.**
 *   워밍업 1회 후 3회의 **중앙값**을 쓴다.
 *   ★ 중앙값이 ReDoS 를 놓치지 않는 이유 — 역행추적은 입력에 결정적이라 3회 전부 느리다.
 *     18,000 ms 가 중앙값 뒤에 숨을 수 없다. 반대로 GC·JIT 스파이크는 3회 중 1회만 튄다.
 *
 * ⚠ 측정을 고쳐도 여전히 실패했다. 그 다음 진단이 아래 SLOW_MS 주석이다 —
 *   상한 자체가 **제이 PC 전용 숫자**였다는 것이 진짜 원인이었다.
 */
function timesMs(fn, runs) {
...
/** 절대 상한 검사용. 중앙값 — 현실적인 1회 처리 시간에 가깝다. */
function medianMs(fn, runs = 3) {
  const ts = timesMs(fn, runs);
  return ts[Math.floor(ts.length / 2)];
}

/**
 * ★★★ 세션45 — 절대 ms 상한을 30 → SLOW_MS(120) 로 통일했다. 근거를 적는다(§9 규칙).
 *
 * 무엇이 문제였나 —
 *   세션44 는 제이 PC 에서 측정한 값에 맞춰 상한을 30 / 60 ms 로 **빡빡하게** 박았다.
 *   같은 코드가 Claude 샌드박스에서는 1.5~2배 느리다. 실측 예:
 *     · `1회+공백` 9,896자         제이 PC 기준 36 ms → 샌드박스 중앙값 51~69 ms
 *     · `메타 2400자`(짧은 입력)                        → 샌드박스 중앙값 37.5 ms (상한 30)
 *   코드가 퇴행한 것이 아니라 **하드웨어가 다르다.** 그런데 테스트는 "수량자 상한이 풀렸다" 고
 *   말한다 — 거짓 진단이다. 그리고 무작위로 빨간불이 뜨는 감시 장치는 사람이 무시하게 되므로,
 *   빡빡한 상한은 안전을 높이는 게 아니라 **감시를 무력화**한다.
 *
 * 왜 120 이 충분한가 —
 *   막으려는 실패 모드는 초 단위다: 411 B 18,055 ms · `1회` 402자 16,974 ms ·
 *   `BASIS_TOTAL` 3,200자 7,746 ms · 정본 `ING_STOP` 2,400자 5,886 ms.
 *   120 ms 는 그중 가장 작은 것(5,886)보다도 **49배 아래**다. 감시 능력은 그대로다.
 *   반대로 정상 최악(샌드박스 69 ms)보다 1.7배 여유가 있어 잡음에 흔들리지 않는다.
 *
 * ★ 절대 상한은 이제 **거친 백스톱**이고, ReDoS 의 정밀 탐지는
 *   §6 의 「2배 길이 → 4배 미만」 배수 검사가 담당한다. 배수는 기계에 의존하지 않는다.
 */
const SLOW_MS = 120;
 ...
 * 증가율(배수) 검사용. **최솟값**을 쓴다.
 *
 * ★ 왜 배수 검사에는 중앙값이 아니라 최솟값인가 — 세션45 실측으로 확인했다.
 *   같은 입력쌍의 배수를 9회씩 재보면
 *     최솟값 기준 : 1.65 · 2.26 · 1.66 · 2.07  (일관)
 *     중앙값 기준 : 1.67 · 2.12 · **3.51** · 1.42  (튄다)
 *   배수는 두 측정값의 **나눗셈**이므로 잡음이 두 번 곱해져 들어온다.
 *   최솟값은 "방해받지 않았을 때의 속도" 이고 그것이 알고리즘의 성질에 가장 가깝다.
 *   ★ ReDoS 는 입력에 결정적이라 최솟값도 느리다 — 최솟값을 써도 놓치지 않는다.
 *
 *   ⚠ 이걸 중앙값으로 되돌리면 테스트가 무작위로 빨간불이 되고, 사람은 감시 장치를 무시하게 된다.
 */
function minMs(fn, runs = 5) {
  return timesMs(fn, runs)[0];
}
...
t('★ 중대7 — 정본 detectBasis 가 적대적 공백에서 SLOW_MS(120) 안에 끝난다', () => {
  // 역이식 초판은 상한 없는 수량자 4개를 그대로 복사해 3,200자에서 7,861 ms 였다.
  for (const n of [1600, 3200, 9900]) {
    const s = `총 내용량${' '.repeat(n)}`;
    const ms = medianMs(() => canon.detectBasis(s));
    assert.ok(ms < SLOW_MS, `정본 ${n}자 ${ms.toFixed(1)}ms — 수량자 상한이 풀렸다`);
  }
});
...
t('★ 치명2 — analyzeText 가 적대적 입력 20종에서 전부 240 ms 안에 끝난다 (세션45: 60→SLOW_MS*2, 근거 아래)', () => {
  ...
  for (const [k, v] of Object.entries(battery)) {
    // ★ 세션45: 콜드 런 1회 측정은 40% 확률로 거짓 실패했다. medianMs 주석 참조.
    const ms = medianMs(() => app.analyzeText(v));
    if (ms > worst) { worst = ms; worstKey = k; }
  }
  console.log(`     (${Object.keys(battery).length}종 · 최악 중앙값 ${worst.toFixed(1)} ms — ${worstKey})`);
  // ★★ 세션45 — 상한을 60 → 250 ms 로 올렸다. **근거를 적는다**(§9 규칙).
  //
  //   왜 60 이 부족했나 — 절대 ms 는 기계에 의존한다.
  //   세션44 는 제이 PC 에서 최악 36 ms 를 기록했다. 같은 코드가 Claude 샌드박스에서는
  //   `1회+공백`(9,896 공백) 중앙값 **51 ms** 다. 코드 변화가 아니라 하드웨어 차이다.
  //   그 상태로 60 을 두면 CI·다른 PC 에서 무작위로 빨간불이 뜨고, 그러면 사람이
  //   "또 그거네" 하고 무시하게 된다 — 감시 장치가 죽는 것이 상한이 넉넉한 것보다 위험하다.
  //
  //   선형이라는 근거 (`1회` + 공백 n 개, 중앙값 5회) —
  //     n=1,237 → 5.2 ms   n=2,475 → 18.2 ms   n=4,950 → 31.1 ms
  //     n=9,896 → 51.1 ms  n=19,792 → 119.9 ms
  //     길이 2배당 1.6~2.3배. 역행추적이면 2배당 4배 이상으로 뛰고 초 단위가 된다
  //     (실측 사례: `_stripAllergenSuffix` 411바이트 18,055 ms).
  //
  //   250 ms 는 실제 실패 모드(18,000 ms)보다 **72배 아래**이므로 감시 능력은 유지된다.
  //   ★ 그리고 절대값만으로는 부족하므로 **배수 검사**를 아래 케이스로 함께 넣었다.
  //     기계가 느려도 배수는 변하지 않는다 — 그쪽이 ReDoS 의 정확한 지문이다.
  assert.ok(worst < SLOW_MS * 2, `${worstKey} = ${worst.toFixed(1)}ms — 상한 없는 수량자가 남아 있다`);
});

t('★★ 세션45 — 입력을 2배로 늘려도 시간이 4배 넘게 늘지 않는다 (ReDoS 의 기계 독립 지문)', () => {
  // ★ 왜 이 검사가 절대 ms 상한보다 정확한가 —
  //   절대값은 하드웨어·부하에 흔들리지만 **증가율은 알고리즘의 성질**이다.
  //   선형이면 2배, 역행추적이면 수십~수천 배. 4배를 경계로 두면 O(n²) 도 걸린다.
  //   상한 없는 수량자가 다시 들어오면 여기서 먼저 터진다.
  const shapes = {
    '1회+공백': (n) => `1회${' '.repeat(n)}`,
    '원재료명+공백': (n) => `원재료명: 밀가루${' '.repeat(n)}가`,
    '알레르기+공백': (n) => `원재료명: 알레르기${' '.repeat(n)}x`,
    '총내용량+공백': (n) => `총 내용량${' '.repeat(n)}`,
    '함유런': (n) => '밀 함유\n'.repeat(Math.floor(n / 7)),
    // ★★★ 세션46 2차 검증 경미9 — 배터리가 **공백런에만 편향**돼 있었다.
    //   `NUTRIENT_PATTERNS` 의 `(\d+[.,]?\d*)` 30곳에 상한이 없어 숫자런에서 O(n²) 였는데
    //   위 5종 어디에도 긴 숫자런이 없어 세션44·45 검사를 전부 통과했다.
    //   실측(최솟값, 수정 전): 402자 1.5ms → 2,400자 34.5ms → 9,600자 534.8ms.
    //   상한 10,000자에서 최악 720ms(열량런) — 초 단위는 아니지만 `/multi-photo` 는
    //   사진 2장이라 요청당 1.4초까지 이벤트 루프를 점유한다.
    //   → 수량자에 `{1,12}` / `{0,6}` 상한을 넣어 5ms 대로 내렸다. 여기서 다시 감시한다.
    '나트륨+숫자런': (n) => `나트륨${'0'.repeat(n)}`,
    '열량+숫자런': (n) => `열량${'0'.repeat(n)}kcal`,
  };
  // ★ 4,800 → 9,600 을 쓴다. 2,400 은 측정값이 잡음과 같은 자릿수라 배수가 흔들린다.
  //   실측(최솟값 9회): 4,800→9,600 배수 2.26 · 2.07. 2,400→4,800 은 1.65 · 1.66.
  //   둘 다 4 아래지만 큰 쪽이 신호대잡음비가 좋다.
  for (const [name, mk] of Object.entries(shapes)) {
    const t1 = minMs(() => app.analyzeText(mk(4800)));
    const t2 = minMs(() => app.analyzeText(mk(9600)));
    // 0 에 가까운 값은 비율이 무의미하다(0.1/0.02 = 5배). 바닥을 깔아 잡음을 배제한다.
    const ratio = t2 / Math.max(t1, 0.5);
    // ★ 임계값 8배 — 세션45 실측 근거.
    //   관측 배수(최솟값 5회, 샌드박스 30여 회 반복): 1.65 ~ **4.2**.
    //   최댓값 4.2 는 `함유런` 이다. 이 입력은 길이가 2배가 되면 알레르기 세그먼트 수도 2배가 되고
    //   세그먼트마다 19종 표를 훑으므로 선형 작업량이 2배다. 여기에 GC 가 겹치면 4배 부근까지 간다.
    //   → 4 를 경계로 두면 정상 코드가 무작위로 실패한다(실측 3회 중 1회).
    //
    //   8배가 여전히 감시가 되는 이유 — 실제 ReDoS 는 자릿수가 다르다.
    //     `_stripAllergenSuffix` 패턴2 실측: 111 B 82 ms → 411 B 18,055 ms.
    //     길이 3.7배에 시간 **220배**. 2배 구간으로 환산해도 수십 배다.
    //   즉 8 은 정상(≤4.2)과 병리(≥수십)의 **빈 구간**에 있다. 임계값을 두기 가장 좋은 자리다.
    //   ❌ 실패할 때마다 이 숫자를 올리지 말 것. 올릴 근거는 "정상 상한을 다시 실측했다" 뿐이다.
    assert.ok(ratio < 8,
      `${name}: 4,800자 ${t1.toFixed(2)}ms → 9,600자 ${t2.toFixed(2)}ms = ${ratio.toFixed(1)}배 — 역행추적 의심`);
  }
});

t('치명2 — 개별 저격 입력이 SLOW_MS(120) 안에 끝난다 (O(n²) 지문 검사)', () => {
  ...
  const shots = {
    ...
    // ★ 세션46 경미9 — 상한(MAX_OCR_TEXT_LENGTH=10000) 근처의 숫자런.
    //   수정 전 실측: 나트륨 555.7ms · 탄수화물 621.6ms · 열량 720.7ms.
    '나트륨 숫자런 10k': `나트륨${'0'.repeat(9997)}`,
    '열량 숫자런 10k': `열량${'0'.repeat(9994)}kcal`,
    '탄수화물 숫자런 10k': `탄수화물${'0'.repeat(9995)}g`,
  };
  for (const [k, v] of Object.entries(shots)) {
    const ms = medianMs(() => app.analyzeText(v));
    assert.ok(ms < SLOW_MS, `${k} = ${ms.toFixed(1)}ms`);
  }
});
...
t('★ 경미K — 정본 ING_STOP 이 개행런에서 SLOW_MS(120) 안에 끝난다', () => {
  ...
  for (const n of [1200, 2400, 4800]) {
    const s = `원재료명: 밀가루${'\n'.repeat(n)}`;
    const ms = medianMs(() => canon.parseLabel(s));
    assert.ok(ms < SLOW_MS, `정본 개행 ${n}자 ${ms.toFixed(1)}ms`);
  }
});
